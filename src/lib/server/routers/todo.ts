import { getTodById, getTodo } from "@/lib/api/todo/queries";
...
export const todoRouter = router({
  ...
  createTod: publicProcedure
    .input(insertTodParams)
    .mutation(async ({ input }) => {
      return createTod(input);
    }),
  updateTod: publicProcedure
    .input(updateTodParams)
    .mutation(async ({ input }) => {
      return updateTod(input.id, input);
    }),
  deleteTod: publicProcedure
    .input(todIdSchema)
    .mutation(async ({ input }) => {
